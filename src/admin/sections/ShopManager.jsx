...
const empty = { name: '', medium: 'Watercolor', size: '', price: '', available: true, image: '' };

export default function ShopManager() {
  const [products, setProducts] = useState(() => getLocal('shopPaintings', []));
  ...
  const save = (list) => { setProducts(list); setLocal('shopPaintings', list); };

  const openAdd = () => { setEditId(null); setForm(empty); setShowForm(true); };
  const openEdit = (p) => { setEditId(p.id); setForm({ name: p.name, medium: p.medium, size: p.size, price: p.price, available: p.available, image: p.image }); setShowForm(true); };
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) {
      save(products.map(p => p.id === editId ? { ...p, ...form, price } : p));
    } else {
      save([...products, { ...form, price, id: generateId(), created_at: new Date().toISOString() }]);
    }
    setSuccess(editId ? 'Product updated!' : 'Product added to shop!');
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={s.title}>Shop Products</h2>
          <p style={s.subtitle}>{products.length} products · {products.filter(p => p.available).length} available</p>
        </div>
      ...
      {products.length === 0 && !showForm && (
        <div style={s.empty}>No products yet. Click "Add Product" to add your first.</div>
      )}
        ...
        {products.map(p => (
          <div key={p.id} style={{ backgroundColor: '#fff9f4', borderRadius: 14, overflow: 'hidden', border: '1px solid #f5e6d3', opacity: p.available ? 1 : 0.65 }}>
            ...
            <div style={{ padding: '10px 12px' }}>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: '0.92rem', fontWeight: 600, color: '#2c1810', marginBottom: 2 }}>{p.name}</p>
              ...
              <div style={{ display: 'flex', gap: '0.35rem' }}>
                ...
                <button onClick={() => deleteProduct(p.id)} style={s.smallBtn('#dc2626')}>Del</button>
              </div>
            </div>
          </div>
        ))}
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <div onClick={() => fileRef.current.click()} style={s.uploadZone}
                  ...
                  <p style={{ fontFamily: "'Jost', sans-serif", fontSize: '0.88rem', color: '#8a7060', margin: 0 }}>
                    {uploading ? 'Compressing...' : form.image ? '📷 Change Image' : '📷 Upload Product Photo'}
                  </p>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.9rem' }}>
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Golden Lotus" style={s.input} onFocus={e => e.target.style.borderColor = '#c17f47'} onBlur={e => e.target.style.borderColor = '#f5e6d3'} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input required value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} placeholder="₹9,500" style={s.input} onFocus={e => e.target.style.borderColor = '#c17f47'} onBlur={e => e.target.style.borderColor = '#f5e6d3'} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingTop: 18 }}>
                  <input type="checkbox" id="shopAvail" checked={form.available} onChange={e => setForm(f => ({ ...f, available: e.target.checked }))} style={{ width: 18, height: 18, accentColor: '#c17f47' }} />
                  <label htmlFor="shopAvail" style={{ fontFamily: "'Jost', sans-serif", fontSize: '0.88rem', color: '#2c1810', cursor: 'pointer' }}>Available for purchase</label>
                </div>
              </div>

              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
                <button type="submit" style={s.primaryBtn}>{editId ? 'Save Changes' : 'Add to Shop'}</button>
